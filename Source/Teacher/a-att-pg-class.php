...
// $currentDateTime = strtotime('07:36:00');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // query for fetching CLASS SCHEDULE
    $query1=mysqli_query($conn,"SELECT * FROM `class_sched` WHERE cs_id = '$csid' AND id_tl = '$tlid'");
    ...
    // STUDENT INFORMATION
    $query = "SELECT ns.fname AS fname, ns.mname AS mname, ns.lname AS lname, ns.barangay AS barangay, ns.images AS images, sy.sem_id AS semId FROM new_students AS ns
            LEFT JOIN school_year AS sy ON ns.sy_enrolled = sy.sy_id
            WHERE student_id = ? AND adviser = ? ";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, 'si', $student_id, $advid);
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if ($currentTime >= $cinp && $currentTime < $cinl) {
            ...
            $log_upt_out = "OUT";
            $current_logout = 0;
        } elseif ($currentTime >= $cinl && $currentTime < $cout) {
            $login_date= $currentDate;
            $login_time = $currentTime;
            $cstat = "Late";
            $new_login_count;
            ...
            $checkQuery = "SELECT * FROM attendance_class WHERE student_id = ? and cl_date = ?";
            ...
            if ($checkResult && mysqli_num_rows($checkResult) > 0) {
                ...
                if (($currentTime >= $cinp && $currentTime < $cinl && $l_status == "OUT") || ($currentTime >= $cinl && $currentTime < $cout && $l_status == "OUT")) {
                    // The student is already logged in for afternoon attendance, so update the login count
                    $updateQuery = "UPDATE `attendance_class` SET in_count = $inc + 1, log_status = ? WHERE  student_id = ? and cl_date = ?";
                    ...
                    mysqli_stmt_bind_param($stmt, 'sss',$log_upt_in, $student_id, $login_date);
                    mysqli_stmt_execute($stmt);
                } elseif (($currentTime >= $cinp && $currentTime < $cinl && $l_status == "IN") || ($currentTime >= $cinl && $currentTime < $cout && $l_status == "IN")) {
                    // The student is already logged in for afternoon attendance, so update the login count
                    $updateQuery = "UPDATE `attendance_class` SET out_count = $outc + 1, log_status = ? WHERE  student_id = ? and cl_date = ?";
                    // $insert2 = mysqli_query($conn, "INSERT INTO `logs`(icon,user_id,action) values('fas fa-sign-in','$student_id','Logged $log_upt_out')");
                    $stmt = mysqli_prepare($conn, $updateQuery);
                    mysqli_stmt_bind_param($stmt, 'sss',$log_upt_out, $student_id, $login_date);
                    mysqli_stmt_execute($stmt);
                } elseif (($currentTime >= $cout && $l_status == "IN")) {
                    // The student is already logged in for afternoon attendance, so update the login count
                    $updateQuery = "UPDATE `attendance_class` SET out_count = $outc + 1, log_status = ?, time_out = ? WHERE  student_id = ? and cl_date = ?";
                    // $insert2 = mysqli_query($conn, "INSERT INTO `logs`(icon,user_id,action) values('fas fa-sign-in','$student_id','Logged $log_upt_out')");
                    $stmt = mysqli_prepare($conn, $updateQuery);
                    mysqli_stmt_bind_param($stmt, 'ssss',$log_upt_out, $login_time, $student_id, $login_date);
                    mysqli_stmt_execute($stmt);
                } elseif (($currentTime >= $cout && $l_status == "OUT")) {
                    // The student is already logged in for afternoon attendance, so update the login count
                    $updateQuery = "UPDATE `attendance_class` SET in_count = $inc + 1, log_status = ? WHERE  student_id = ? and cl_date = ?";
                    // $insert2 = mysqli_query($conn, "INSERT INTO `logs`(icon,user_id,action) values('fas fa-sign-in','$student_id','Logged $log_upt_out')");
                    $stmt = mysqli_prepare($conn, $updateQuery);
            ...
            } elseif ($currentTime >= $cinp && $currentTime < $cinl) {
                // The student is not logged in for afternoon attendance, so insert a new record
                $insertQuery = "INSERT INTO `attendance_class` (cl_date, student_id, id_tl,time_in,attendance_status,in_count,out_count,log_status,sem_id) VALUES (?, ?, ?, ?, ?,?,?,?,?)";
                $insert2 = mysqli_query($conn, "INSERT INTO `logs`(icon,user_id,action) values('fas fa-sign-in','$student_id','Logged $log_status')");
                $stmt = mysqli_prepare($conn, $insertQuery);
                mysqli_stmt_bind_param($stmt, 'ssisssssi', $login_date, $student_id,$idtl, $login_time,$cstat, $new_login_count, $current_logout, $log_status, $semId);
                mysqli_stmt_execute($stmt);
            } elseif ($currentTime >= $cinl && $currentTime < $cout) {
                // The student is not logged in for afternoon attendance, so insert a new record
                $insertQuery = "INSERT INTO `attendance_class` (cl_date, student_id, id_tl,time_in,attendance_status,in_count,out_count,log_status,sem_id) VALUES (?, ?, ?, ?, ?,?,?,?,?)";
        ...
        $data = array(
            ...
            'l_status' => isset($row2['log_status']) ? ($l_status == 'IN' ? $log_upt_out : $log_upt_in) : $log_status // Pass the attendance status to the client
        );
...
<?php 
    ...
    $advid1 = $_GET['advid'];
        ...
        // query for fetching CLASS SCHEDULE
        $query1=mysqli_query($conn,"SELECT * FROM `class_sched` WHERE cs_id = '$csid1' AND id_tl = '$tlid1'");
        while($row1=mysqli_fetch_array($query1))
            {
                $logout = $row1['log_out'];
    ...
    $day_out_time = date("H:i", strtotime($logout . " +5 minutes"));
    ...
    if (time() > $day_out_time) {
        ...
        $query = "SELECT ns.student_id, sy.sem_id AS semId FROM new_students AS ns
                LEFT JOIN school_year AS sy ON ns.sy_enrolled = sy.sy_id            
                LEFT JOIN attendance_class AS ag ON ns.student_id = ag.student_id AND ag.cl_date = ?
                WHERE (ns.adviser = '$advid1') AND ag.student_id IS NULL"; // Added condition to check for the specific date or if no record exists for the student
        ...
        if($result && mysqli_num_rows($result) > 0){
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $semId = $row['semId'];
                $insertQuery = "INSERT INTO attendance_class (cl_date, student_id,id_tl,time_in,time_out,attendance_status,sem_id) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_prepare($conn, $insertQuery);
                mysqli_stmt_bind_param($stmt, 'sssssss',$dateDaily, $student_id, $tlid1, $not_log, $not_log, $A_status, $semId );
                mysqli_stmt_execute($stmt);
            }
        }
...
<header>
    <?php
            //query for FETCHING SCHOOL INFORMATION
    $siquery = mysqli_query($conn,"SELECT * FROM school_profile");
    if(mysqli_num_rows($siquery)){
        $row = mysqli_fetch_assoc($siquery);
        $slogo = $row['image'];
        $sname = $row['name'];
    }
    ?>
    <img src="<?php echo $slogo?>" alt="" width="100" height="100" class="slogo" srcset="">
    <div id="liveDateTime" class="text-white"></div>
    <h1>Welcome To <?php echo $sname?></h1>
</header>
...
<main>
    ...
    for (var i = currentImages.length - 1; i >= 0; i--) {
        ...
        function resetCardData() {
            ...
            $('#fname').text('');
            $('#lname').text('');
            $('#status').text(''); // Show 'Loading...' for morning status
        }
