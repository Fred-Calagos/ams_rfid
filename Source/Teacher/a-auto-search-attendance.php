...
$pagi = '';
    ...
    if(isset($_POST["query"]))
        ...
        LEFT JOIN teacher AS advId ON ns.adviser = advId.t_id
        LEFT JOIN teacher_info AS adv ON advId.t_id = adv.t_id
        WHERE (advId.t_id = ns.adviser) AND (adv.t_rfid = '$username') AND (ag.student_id LIKE '%".$search."%'
    ...
    LEFT JOIN teacher AS advId ON ns.adviser = advId.t_id
    LEFT JOIN teacher_info AS adv ON advId.t_id = adv.t_id
    WHERE advId.t_id = ns.adviser AND adv.t_rfid = '$username'
    ...
    if(mysqli_num_rows($result) > 0)
        ...
        {
            $n++;
            $formatted_date = date("F j, Y", strtotime($row_morning['att_date']));
            $output .= '
            <tr>
                <td>'.$n.'</td>
