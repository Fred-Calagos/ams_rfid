                            ...
                            <select name="idtl" >
                                ...
                                <?php
                                    $res = mysqli_query($conn, "SELECT tl.*, ti.t_rfid AS trfid, tl.id_tl AS idtl, sub.sub_name AS sname, sub.s_id AS sId, CONCAT(adv.t_ln,', ',adv.t_fn,' ',adv.t_mn) AS advName,gr.stud_grade AS grade, hs.hs_name AS hsName FROM teaching_load AS tl
                                                                        ...
                                                                        WHERE ti.t_id = tl.t_id AND ti.t_rfid = '$username' and sub.s_id = tl.s_id AND gr.hs_id = '1'");
                ...
                if(isset($_POST['submit2'])) {
                    ...
                    $fetchingStudents = mysqli_query($conn, "SELECT 
                                        ...
                                        LEFT JOIN school_year AS sy ON ns.sy_enrolled = sy.sy_id
                                         ...
                                         GROUP BY ag.student_id, gr.stud_grade, sc.section_name, sy.sy_name") 
...
if ($student) {
    ...
    <!-- SECOND ROW -->
    <tr>
        <td colspan="2"><label for="">School Year</label></td>
        <td colspan="8">
            <input type="text" name="sy" class="puts1" id="sy" value="<?php echo $syName?>" readonly>
        </td>
        <td colspan="4" style="text-align:right"><label for="">Grade Level</label></td>
        <td colspan="5">
...
do {
    ...
    for ($j = 1; $j <= $totalDaysInMonth; $j++) {
        ...
        $fetchingStudentsAttendance = mysqli_query($conn, "SELECT * FROM attendance_class WHERE student_id = '$student_id' AND cl_date = '$dateOfAttendance' AND (track = empty AND strand = empty) ") or die(mysqli_error($conn));
...
?>
    ...
    function print_div() {
        ...
        printWindow.document.write('td { border: 2px solid black; width: 100%; white-space:nowrap;padding:5px;}'); // Adjusted width to auto
