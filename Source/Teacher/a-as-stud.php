...
$pagi = '';
    ...
    if(isset($_POST["query"]))
        ...
        LEFT JOIN teacher AS advId ON st.adviser = advId.t_id
        LEFT JOIN teacher_info AS adv ON advId.t_id = adv.t_id

        WHERE arc_stat = 1 AND (advId.t_id = st.adviser) AND (adv.t_rfid = '$username') AND (student_id LIKE '%".$search."%'
    ...
    LEFT JOIN teacher AS advId ON st.adviser = advId.t_id
    LEFT JOIN teacher_info AS adv ON advId.t_id = adv.t_id
    WHERE advId.t_id = st.adviser AND adv.t_rfid = '$username' AND arc_stat = 0
    ORDER BY fname ASC";
    }
    ...
    if(mysqli_num_rows($result) > 0)
        ...
        while($row_morning = mysqli_fetch_array($result))
        {
            ...
            <tr>
                <td>'.$n.''.".".'</td>
                <td><img src='.$row_morning['images'].' class="img_tbl" alt="Student Image"></td>
                ...
                <td><a href="s-edit.php?e_id='.$row_morning['id'].'&name='.$row_morning['fname'].'"><i class="fas fa-edit btn_edit1"></i></a>
                    <a href="m-student.php?d_id='.$row_morning['id'].'"><i class="fas fa-trash btn_delete1"></i></a>
                </td>
            </tr>
            </form>
            ';
