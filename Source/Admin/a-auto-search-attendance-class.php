...
if(isset($_POST["query"]))
{
    ...
    WHERE CONCAT(ns.lname,', ',ns.fname,' ',ns.mname) LIKE '%".$search."%'
    ";
}
else
{
...
if(mysqli_num_rows($result) > 0)
{
    ...
    while($row_morning = mysqli_fetch_array($result))
    {
        ...
        <tr>
            <td>'.$n.'</td>
            <td>'.(isset($row_morning['full_name']) ? $row_morning['full_name'] : 'N/A').'</td>
            <td>'.(isset($row_morning['clDate']) ? $row_morning['clDate'] : 'N/A').'</td>
            <td>'.(isset($row_morning['tName']) ? $row_morning['tName'] : 'N/A').'</td>
            <td>'.(isset($row_morning['time_in']) ? $row_morning['time_in'] : 'N/A').'</td>
            <td>'.(isset($row_morning['time_out']) ? $row_morning['time_out'] : 'N/A').'</td>
            <td>'.(isset($row_morning['in_count']) ? $row_morning['in_count'] : 'N/A').'</td>
            <td>'.(isset($row_morning['out_count']) ? $row_morning['out_count'] : 'N/A').'</td>
            <td>'.(isset($row_morning['attendance_status']) ? $row_morning['attendance_status'] : 'N/A').'</td>
            <td><a href="a-attendance-class.php?clid='.(isset($row_morning['ca_id']) ? $row_morning['ca_id'] : '').'"><i class="fas fa-edit btn_edit1"></i></a></td>
        </tr>
        ';
    }
    $output .= '
