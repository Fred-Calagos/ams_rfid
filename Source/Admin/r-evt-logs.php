                        ...
                        <ul>
                            <li>
                                <select name="event">
                                    ...
                                    <?php
                                    $sql = "SELECT * FROM event_sched ORDER BY event_name asc";
                    ...
                    <div class="printable">
                        ...
                        <?php 
                                   if (isset($_POST["submit"])) {
                                    ...
                                    $i=0;
                                    $res = mysqli_query($conn, "SELECT ag.stud_rfid as student_id,
                                    SUM(ag.m_log_in) AS mti, 
                                    SUM(ag.m_log_out) AS mto, 
                                    SUM(ag.aft_log_in) AS aftti,
                                    SUM(ag.aft_log_out) AS aftto,
                                    SUM(ag.eve_log_in) AS eveti,
                                    SUM(ag.eve_log_out) AS eveto,
                                    es.event_name as en, es.event_date as ed,
                                    ...
                                    SUM(ag.eve_log_in) AS eveti,
                                    SUM(ag.eve_log_out) AS eveto,
                                    es.event_name AS event_n,
                                    GROUP_CONCAT(DISTINCT ns.lname,', ',ns.fname,' ',ns.mname) as full_name
                                    FROM `attendance_event` AS ag
                                    LEFT JOIN new_students as ns ON ag.stud_rfid = ns.student_id
                                    LEFT JOIN event_sched as es ON ag.eid = es.id 
                                    GROUP BY ag.stud_rfid, es.event_name
                                    ORDER BY full_name ASC
                                    ");
                                        ?>
                                    <thead>
                                        <tr>
                                            <th colspan="9">Event Log Attendance Report</th>
                                        </tr>
                                        <tr>
                                            <th rowspan="2">No.</th>
