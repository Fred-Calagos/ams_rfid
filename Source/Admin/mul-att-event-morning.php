...
$attendanceStart = '06:00:00';
$attendanceEnd = '13:00:00';
...
if ($currentTime < $attendanceStart || $currentTime > $attendanceEnd) {
    ...
    echo json_encode(['error' => 'The attendance will start at 7:00 AM and end at 12:00 PM.']);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $morningAttendanceStart = '06:00:00';
        $morningCutOff = '7:30:00';
        $morningCutOffEnd = '12:00:00';

        $logoutMorning = '12:01:00';
        $logoutMorningEnd = '13:00:00';
        ...
        if ($currentTime >= $morningAttendanceStart && $currentTime <= $morningCutOff) {
            $date = $currentDate;
            $time_in_morning = $currentTime;
        } elseif ($currentTime >= $morningCutOff && $currentTime <= $morningCutOffEnd) {
            $date = $currentDate;
            $time_in_morning = $currentTime;
        } else {
            $date = $currentDate;
        ...
        $checkQuery = "SELECT * FROM `event_attendance_morning` WHERE student_id = ? AND date = ?";
        ...
        if ($checkResult && mysqli_num_rows($checkResult) > 0) {
            if ($currentTime < $logoutMorning) {
                echo "You cannot log out yet. Please wait until $logoutMorning.";
            } else {
                $updateQuery = "UPDATE `event_attendance_morning` SET time_out_morning = ? WHERE student_id = ? AND time_out_morning IS NULL";
                $stmt = mysqli_prepare($conn, $updateQuery);
                mysqli_stmt_bind_param($stmt, 'ss', $currentTime, $student_id);
                mysqli_stmt_execute($stmt);
            } 
        } elseif ($currentTime >= $logoutMorning || $currentTime <= $logoutMorningEnd) {
            $insertQuery = "INSERT INTO `event_attendance_morning` (student_id, images, fname, mname, lname, barangay, date, time_in_morning) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $insertQuery);
            mysqli_stmt_bind_param($stmt, 'ssssssss', $student_id, $images, $fname, $mname, $lname, $barangay, $date, $time_in_morning);
            mysqli_stmt_execute($stmt);
        } else {
            $insertQuery = "INSERT INTO `event_attendance_morning` (student_id, images, fname, mname, lname, barangay, date, time_in_morning) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $insertQuery);
            mysqli_stmt_bind_param($stmt, 'sssssss', $student_id, $images, $fname, $mname, $lname, $barangay, $date, $time_in_morning);
            mysqli_stmt_execute($stmt);
        }
        ...
        $data = array(
            ...
            'images' => $images,
            'date' => $date,
            'time_in_afternoon' => $time_in_morning
        );
...
<script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data) {
                        ...
                        if (data.time_out_morning) {
                            $('#time_out_morning').text('Logout Time: ' + data.time_out_morning);
                            isLoggedOut = true;
    ...
    function resetCardData() {
        ...
        $('#fname').text('');
        $('#lname').text('');
        $('#morning_status').text('ATTENDANCE NOT ALLOWED OUTSIDE MORNING HOURS');
    }

    setTimeout(resetCardData, 3000);

    // Check attendance end time
    var attendanceEndTime = '13:00:00'; // Replace with your actual attendance end time
