...
$attendanceStart = '13:00:00';
$attendanceEnd = '18:00:00';
...
if ($currentTime < $attendanceStart || $currentTime > $attendanceEnd) {
    ...
    echo json_encode(['error' => 'The attendance will start at 1:00 PM and end at 5:00 PM.']);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $afternoonAttendanceStart = '13:00:00';
        $afternoonCutOff = '13:30:00';
        $afternoonCutOffEnd = '17:00:00';

        $logoutAfternoon = '17:01:00';
        $logoutAfternoonEnd = '18:00:00';
        ...
        if ($currentTime >= $afternoonAttendanceStart && $currentTime <= $afternoonCutOff) {
            $date = $currentDate;
            $time_in_afternoon = $currentTime;
        } elseif ($currentTime >= $afternoonCutOff && $currentTime <= $afternoonCutOffEnd) {
            $date = $currentDate;
            $time_in_afternoon = $currentTime;
        } else {
            $date = $currentDate;
        ...
        $checkQuery = "SELECT * FROM `event_attendance_afternoon` WHERE student_id = ? AND date = ?";
        ...
        if ($checkResult && mysqli_num_rows($checkResult) > 0) {
            if ($currentTime < $logoutAfternoon) {
                echo "You cannot log out yet. Please wait until $logoutAfternoon.";
            } else {
                $updateQuery = "UPDATE `event_attendance_afternoon` SET time_out_afternoon = ? WHERE student_id = ? AND time_out_afternoon IS NULL";
                $stmt = mysqli_prepare($conn, $updateQuery);
                mysqli_stmt_bind_param($stmt, 'ss', $currentTime, $student_id);
                mysqli_stmt_execute($stmt);
            } 
        } elseif ($currentTime >= $logoutAfternoon || $currentTime <= $logoutAfternoonEnd) {
            $insertQuery = "INSERT INTO `event_attendance_afternoon` (student_id, images, fname, mname, lname, barangay, date, time_in_afternoon) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $insertQuery);
            mysqli_stmt_bind_param($stmt, 'ssssssss', $student_id, $images, $fname, $mname, $lname, $barangay, $date, $time_in_afternoon);
            mysqli_stmt_execute($stmt);
        } else {
            $insertQuery = "INSERT INTO `event_attendance_afternoon` (student_id, images, fname, mname, lname, barangay, date, time_in_afternoon) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $insertQuery);
            mysqli_stmt_bind_param($stmt, 'sssssss', $student_id, $images, $fname, $mname, $lname, $barangay, $date, $time_in_afternoon);
            mysqli_stmt_execute($stmt);
        }
...
<script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data) {
                        ...
                        if (data.time_out_afternoon) {
                            $('#time_out_afternoon').text('Logout Time: ' + data.time_out_afternoon);
                            isLoggedOut = true;
    ...
    function resetCardData() {
        ...
        $('#fname').text('');
        $('#lname').text('');
        $('#afternoon_status').text('ATTENDANCE NOT ALLOWED OUTSIDE AFTERNOON HOURS');
    }

    setTimeout(resetCardData, 3000);

    // Check attendance end time
    var attendanceEndTime = '18:00:00'; // Replace with your actual attendance end time
